...
interface HeaderProps {
  ...
  onOpenChooseFormat?: () => void;
  headerRef?: React.RefObject<HTMLElement>;
  chooseFormatOpen?: boolean;
}

const Header: React.FC<HeaderProps> = ({
  onReloadFileConverter,
  onOpenChooseFormat,
  headerRef,
  chooseFormatOpen,
}) => {
 ...
 bg-[linear-gradient(90deg,rgb(0,255,198)_0%,rgb(255,255,255)_50%,rgb(91,0,255)_100%)]
  ...
  "
      >
        ...
        <div className="mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-[53px] w-full">
            ...
            <nav className="hidden lg:flex space-x-6 items-center">
              ...
              <div>
                <button
                  className={`hover:text-black text-gray-700 dark:text-gray-300 dark:hover:text-white px-3 py-2 rounded-3xl text-base font-medium transition-colors flex items-center gap-2`}
                  onClick={onOpenChooseFormat}
                  aria-haspopup="true"
                >
                  {chooseFormatIcon}
                  {t('choose_format')}
              ...
              {NAV_LINKS.filter((link) => link.name !== t('choose_format')).map(
                (link) => (
                  <a
                    key={link.name}
                    href={link.href}
        ...
        {menuOpen && (
          <>
            <div className="lg:hidden bg-red-700 absolute mt-1 top-full left-0 right-0 bg-gradient-to-b from-slate-200 via-slate-100 to-white border-cyan-400 border rounded-2xl lg:rounded-b-[50px] shadow-sm animate-fade-in-down z-40 dark:bg-none dark:bg-[rgb(100,116,139)]">
              ...
              <nav className="flex flex-col px-4 py-2 space-y-1 relative z-40">
                <div className="relative">
                  {/* Nút choose_format cho mobile có thể gọi onOpenChooseFormat nếu muốn hỗ trợ mobile */}
                  <button
                    className={`text-gray-700 hover:text-primary-600 px-3 py-2 rounded-3xl text-base font-medium transition-colors flex items-center gap-3 dark:text-white dark:hover:text-primary-300 w-full justify-start`}
                    onClick={onOpenChooseFormat}
                    aria-haspopup="true"
                  >
                    {chooseFormatIcon}
                ...
                {NAV_LINKS.filter(
                  (link) => link.name !== t('choose_format')
                ).map((link) => (
                  <a
                    key={link.name}
                    href={link.href}
            ...
            {/* Dropdown đã chuyển ra ngoài Header */}
          </>
        )}
      </header>
    </div>
